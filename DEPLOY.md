...
**Render** 하나로 백엔드 + 프론트엔드를 함께 배포하는 방법을 설명합니다.
...
> ⚠️ **변경**: 이제 Render 하나만으로 백엔드 + 프론트엔드가 함께 배포됩니다. Vercel은 필요하지 않습니다.

---

## 💡 주의사항 (Free Tier 제약)
*   **백엔드 수면 모드**: Render의 무료 플랜은 약 15분간 접속이 없으면 서버가 잠듭니다. 이 경우 첫 접속 시 서버가 깨어나는 데 약 30초~1분 정도 걸릴 수 있습니다. (앱이 고장 난 것이 아니니 잠시만 기다려주세요!)
*   **보안**: 현재는 모든 도메인에서의 접속을 허용(`CORS *`)하고 있습니다. 필요한 경우 `backend/index.js`의 CORS 설정을 변경하세요.
