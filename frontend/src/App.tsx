...
const CommentRow: React.FC<{ comment: Comment; isPinnedSection: boolean; currentRank: number; change: { type: string, rankDiff: number, likesDiff: number }; isPinned: boolean; isExpanded: boolean; toggleExpand: (id: number) => void; togglePin: (id: number) => void; showToast: (msg: string) => void; url: string; searchTerm: string; }> = React.memo(({ comment, isPinnedSection, currentRank, change, isPinned, isExpanded, toggleExpand, togglePin, showToast, url, searchTerm }) => {
  ...
  return (
    <div style={{ animationDelay: `${delay}ms` }} className={rowClasses}>
      <div className="flex items-center sm:items-start gap-2 sm:gap-4 w-full sm:w-auto sm:flex-col sm:flex-shrink-0">
        <div className="flex flex-row sm:flex-col items-center sm:justify-start gap-1.5 sm:gap-0 sm:w-10 sm:pt-1.5">
          <span className={`text-xl sm:text-2xl font-bold tabular-nums spring-transition ${currentRank <= 3 && !isPinnedSection ? 'text-brand' : 'text-gray-200 dark:text-gray-800 group-hover:text-gray-400 dark:group-hover:text-gray-600'}`}>{currentRank.toString().padStart(2, '0')}</span>
          <div className="flex flex-row sm:flex-col items-center gap-1 sm:mt-1">{change.type === 'up' && (<div className="flex items-center gap-0.5"><ArrowUp className="w-2.5 h-2.5 stroke-[4] text-green-600 dark:text-green-400" /><span className="text-[9px] font-black text-green-600 dark:text-green-400 tabular-nums">{change.rankDiff}</span></div>)}{change.type === 'down' && (<div className="flex items-center gap-0.5 opacity-40"><ArrowDown className="w-2.5 h-2.5 stroke-[4] text-red-500 dark:text-red-500" /><span className="text-[9px] font-black text-red-500 dark:text-red-400 tabular-nums">{change.rankDiff}</span></div>)}</div>
        </div>
        <div className="relative flex-shrink-0 sm:pt-1"><a href={`https://www.sooplive.com/station/${comment.userId}`} target="_blank" rel="noopener noreferrer"><img src={comment.profileImage} alt="" className="w-9 h-9 sm:w-10 md:w-12 sm:h-10 md:h-12 rounded-xl object-cover border-2 border-[#f0f0f0] dark:border-gray-800/50 shadow-sm spring-transition relative z-10 group-hover:scale-105" loading="lazy" onError={(e) => { e.currentTarget.src = 'https://res.sooplive.com/images/station/img_profile_default.png'; }} /></a></div>
        <div className="flex-1 min-w-0 sm:hidden">
          <div className="flex items-center gap-1.5"><span className="font-bold text-gray-900 dark:text-white text-[14px] truncate">{comment.author}</span><div className="flex items-center gap-1 px-1.5 py-0.5 rounded-lg text-[9px] font-black bg-white dark:bg-gray-800 text-gray-500 border border-gray-100 dark:border-gray-700 tabular-nums flex-shrink-0">{comment.likes.toLocaleString()}</div>{change.likesDiff > 0 && <span className="text-[9px] font-black text-green-500 dark:text-green-400 animate-pulse tabular-nums">+{change.likesDiff}</span>}{change.likesDiff < 0 && <span className="text-[9px] font-black text-red-500 dark:text-red-400 animate-pulse tabular-nums">{change.likesDiff}</span>}</div>
          <div className="text-[9px] font-bold text-gray-400 dark:text-gray-500 truncate">@{comment.userId}</div>
        </div>
      </div>
