    ...
    <div style={{ animationDelay: `${delay}ms` }} className={rowClasses}>
      ...
      <div className="flex items-center gap-2 sm:hidden flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1"><a href={`https://www.sooplive.com/station/${comment.userId}`} target="_blank" rel="noopener noreferrer" className="font-bold text-gray-900 dark:text-white text-[13px] truncate hover:underline">{comment.author}</a><div className="flex items-center gap-0.5 px-1 py-0.5 rounded-lg text-[8px] font-black bg-white dark:bg-gray-800 text-gray-500 border border-gray-100 dark:border-gray-700 tabular-nums flex-shrink-0">{comment.likes.toLocaleString()}</div>{change.likesDiff > 0 && <span className="text-[8px] font-black text-green-500 dark:text-green-400 animate-pulse tabular-nums">+{change.likesDiff}</span>}{change.likesDiff < 0 && <span className="text-[8px] font-black text-red-500 dark:text-red-400 animate-pulse tabular-nums">{change.likesDiff}</span>}</div>
          <div className="text-[8px] font-bold text-gray-400 dark:text-gray-500 truncate">@{comment.userId}</div>
        </div>
        <div className="flex items-center gap-1 ml-auto flex-shrink-0 opacity-0 group-hover:opacity-100 spring-transition">
          <div className="flex items-center gap-1">
            <button aria-label={isExpanded ? "댓글 접기" : "댓글 펼치기"} onClick={() => toggleExpand(comment.id)} className={`${btnBase} ${isExpanded ? 'bg-brand text-white' : 'bg-white dark:bg-gray-800 text-gray-400 dark:text-gray-500 hover:text-brand border border-gray-100 dark:border-gray-700'}`}>{isExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}</button>
            <a aria-label="원문 보기" href={`${url.split('#')[0]}#comment_noti${comment.id}`} target="_blank" rel="noopener noreferrer" className={`${btnBase} bg-white dark:bg-gray-800 text-gray-400 dark:text-gray-500 hover:text-brand border border-gray-100 dark:border-gray-700`} title="원문 보기"><ExternalLink className="w-3 h-3" /></a>
          </div>
          <button aria-label={isPinned ? "고정 해제" : "상단 고정"} onClick={() => togglePin(comment.id)} className={`${btnBase} ${isPinned ? 'bg-brand text-white' : 'bg-white dark:bg-gray-800 text-gray-300 dark:text-gray-600 hover:text-brand border border-gray-100 dark:border-gray-700'}`} title={isPinned ? "고정 해제" : "상단 고정"}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`w-3 h-3 spring-transition ${isPinned ? 'fill-current rotate-45' : 'rotate-0'}`}><circle cx="12" cy="4" r="2.5"/><path d="M8 8h8l-2 11H10L8 8z"/><path d="M12 19v3"/></svg></button>
        </div>
      </div>
...
{change.likesDiff < 0 && <span className="text-[10px] font-black text-red-500 dark:text-red-400 animate-pulse tabular-nums">{change.likesDiff}</span>}
          ...
          <div className="flex items-center gap-1.5 ml-4 flex-shrink-0 pt-0.5">
            ...
            <button aria-label={isPinned ? "고정 해제" : "상단 고정"} onClick={() => togglePin(comment.id)} className={`${btnBase} ${isPinned ? 'bg-brand text-white' : 'bg-white dark:bg-gray-800 text-gray-300 dark:text-gray-600 hover:text-brand border border-gray-100 dark:border-gray-700 opacity-0 group-hover:opacity-100'}`} title={isPinned ? "고정 해제" : "상단 고정"}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`w-3.5 h-3.5 spring-transition ${isPinned ? 'fill-current rotate-45' : 'rotate-0'}`}><circle cx="12" cy="4" r="2.5"/><path d="M8 8h8l-2 11H10L8 8z"/><path d="M12 19v3"/></svg></button>
          </div>
        </div>
